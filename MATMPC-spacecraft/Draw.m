...
switch settings.model
    ...
    case 'simpleSpacecraft'
        ...
        nexttile(2);  % distance
        plot(time,vecnorm(state_sim(:,1:3),2,2));
        grid on
        ylabel('$d$ [m]','interpreter','latex');

        nexttile(3);  % linear vel
        plot(time,state_sim(:,4:6));
        grid on
        ...
        plot(time,controls_MPC);
        grid on
        ylabel('$command$','interpreter','latex');
        %legend('$a_1$','$a_2$','$a_3$')

        nexttile(4,[2,1])
        plot3(state_sim(:,1),state_sim(:,2),state_sim(:,3),'b','Linewidth',2)
        hold on
        plot3(10*[0 input.od(1)],10*[0 input.od(2)],10*[0 input.od(3)],'r')
